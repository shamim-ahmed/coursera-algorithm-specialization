...
public class TravellingSalesmanProblem {
  private static class Impl {
    ...
    void preprocess() {
      // This priority queue will contain pairs (importance, node) with the least important node in
      // the head
    ...
    void add_edge(int side, int u, int v, Long c) {
      for (int i = 0; i < adj[side][u].size(); ++i) {
        ...
        if (w == v) {
          Long cc = Long.min(cost[side][u].get(i), c);
    ...
    Long shortcut(int v) {
      ...
      Long importance =
          (shortcuts - adj[0][v].size() - adj[1][v].size()) + neighbors + shortcutCover + vlevel;
      return importance;
    }
  ...
  // Returns the length of the shortest circular path visiting all the nodes of the graph at least
  // once
  ...
  // Reads the road network and returns the adjacency matrix of the graph on the nodes we are
  // interested in
