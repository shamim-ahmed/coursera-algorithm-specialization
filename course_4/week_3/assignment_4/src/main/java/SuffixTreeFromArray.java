...
public class SuffixTreeFromArray {
  ...
  Map<Integer, List<Edge>> SuffixTreeFromSuffixArray(int[] suffixArray, int[] lcpArray,
      final String text) {
  ...
  public void run() throws IOException {
    ...
    while (stackSize > 0) {
      ...
      result.add(suffixTree.get(node).get(edgeIndex).start + " "
          + suffixTree.get(node).get(edgeIndex).end);
